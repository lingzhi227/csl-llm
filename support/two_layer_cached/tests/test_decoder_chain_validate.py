...
def test_reset_retains_physical_second_slot_but_requires_logical_single_slot(tmp_path):
    initial, reference, normal, policy = fixture()
    retained = {f'layer_{layer}_cache_{kind}': np.ones((2, 2, 64), np.float32)
                for layer in (0, 1) for kind in ('k', 'v')}
    normal['ready'].fill(5); normal['progress'].fill(6)
    bad_visibility = False
    def mutation(point, name, raw):
        if name == 'progress':
            return np.array([6], '<u4').tobytes()
        if name == 'boundary.invocation':
            return np.array([3], '<u4').tobytes()
        if point in ((32, 14), (43, 14), (96, 14), (107, 14)):
            if name in ('kv.keys', 'kv.values'):
                values = np.frombuffer(raw, '<f4').copy()
                if name == 'kv.keys': values.reshape(64, 32)[:, 1] = 1
                else: values.reshape(32, 64)[1] = 1
                return values.tobytes()
            if bad_visibility and name == 'kv.valid':
                return np.array([2], '<u2').tobytes()
        return raw
    assert validate(DiagnosticMemory(mutation), initial, reference, normal, policy,
                    tmp_path, invocation=3, retained_cache=retained)['passed']
    bad_visibility = True
    with pytest.raises(AssertionError):
        validate(DiagnosticMemory(mutation), initial, reference, normal, policy,
                 tmp_path, invocation=3, retained_cache=retained)
