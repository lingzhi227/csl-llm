...
def validate(core, expected_initial, reference, normal, policy, output, *, invocation=1, retained_cache=None):
    import numpy as np
    ...
    if type(invocation) is not int or not 1 <= invocation <= 65535:
        raise ValueError('An explicit nonwrapping invocation is required')
    ...
    np.testing.assert_array_equal(progress, np.full((20, 129), 2*invocation, np.uint32))
    np.testing.assert_array_equal(normal['ready'], np.full(2580, 2*invocation-1, np.uint32))
    ...
    for layer in (0, 1):
        ...
        for head in range(2):
            ...
            if retained_cache is not None:
                # Reset leaves old physical slot one intact but logically
                # invisible. Its values belong to the independent cached prefix.
                for name in ('cache_k', 'cache_v'):
                    assert retained_cache[prefix+name].shape == (2, 2, 64)
                compare(f'{prefix}retained_cache_k_{head}', keys[0, :, 1], retained_cache[prefix+'cache_k'][head, 1])
                compare(f'{prefix}retained_cache_v_{head}', values[0, 1], retained_cache[prefix+'cache_v'][head, 1])
                first_untouched = 2
            assert not keys[0, :, first_untouched:].view(np.uint32).any() and not keys[1:].view(np.uint32).any()
            assert not values[0, first_untouched:].view(np.uint32).any() and not values[1:].view(np.uint32).any()
    cycles = {}
    ...
    return dict(passed=True, checks=checks, cycles=cycles,
        ...
        scope=('Original position-zero two-layer final core, all initializers, completion, KV and protocol states. '
               'Retained physical slot-one checks after reset.' if retained_cache is not None else
               'Original single position-zero two-layer device chain, all initializers, completion, KV and protocol states. Not cached/reset or full-model inference.'))
