...
def execute(root,timeout):
    ...
    result=dict(success=False,sdk_sha256=m['sdk_sha256'],manifest_sha256=sha(root/'manifest.json'),command=cmd,timeout_seconds=timeout,rss_kind='Sum of process-tree resident pages, may double-count shared mappings',peak_rss_kib=0)
    ...
    with (root/'sdk.log').open('w') as log:
        ...
        try:
            while process.poll() is None:
                ...
                if time.monotonic()-start>timeout or rss>24*1024*1024:raise TimeoutError('Probe resource/time budget reached')
                time.sleep(1)
            ...
            result['cleanup_signals']=cleanup_tree(known)
            result['after_cleanup_identities']={str(pid):process_identity(pid) for pid in known}
