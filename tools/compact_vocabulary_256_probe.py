...
def prepare(parent, project, source, count, representation='packed'):
    if not 1 <= count <= 256:
        raise ValueError('This bounded scale experiment permits at most256 static tiles')
    ...
    shutil.copy2(source / 'tools/partition_executor.py', out / 'executor.py')
    shutil.copy2(source / 'tools/partition_encoder.py', out / 'encoder.py')
    ...
    (out / 'config.json').write_text(json.dumps(dict(parent=parent.name,
        ...
        static_tile_indices=list(range(count)), matrix_tiles=9496, representation=representation,
        scope='Compile-only full193x50 vocabulary program, first declared physical row-major tiles statically initialized from original compact byte strings; all remaining matrix weights zero. Exact allocated ELF coordinate audit, no runtime or full initialization/model numerical acceptance.'), indent=2)+'\n')
...
def worker(root):
    ...
    for path in sorted((root / 'out/bin').glob('*.elf')):
        ...
        for x, y in selected:
            ...
            wanted = expected[index*28672:(index+1)*28672] if index < count else bytes(28672)
...
if __name__ == '__main__':
    ...
    if args.from_fixture:
        prepare(args.from_fixture.resolve(), args.project_root.resolve(), args.source_root.resolve(), args.tiles, args.representation)
    elif args.worker:
