"""Complete native layer0 candidate with persistent KV and reset diagnostics."""
import argparse
import datetime
import hashlib
import json
...
def geometry():
    from csl_llm.regions import Region
    return {
        ...
        'q': [Region(54, g, 8, 1) for g in range(7)],
        'k': [Region(54, 7, 8, 1)],
        'v': [Region(54, 8, 8, 1)],
        'o': [Region(54, 9+g, 8, 1) for g in range(7)],
        'kv': [Region(32, 14, 11, 6), Region(43, 14, 11, 6)],
    }


def make_layout(regions):
    ...
    for name, rs in regions.items():
        for group, region in enumerate(rs):
            for ordinal in range(region.participants):
                ...
                destination = regions['gate'][group].point(0) if name == 'up' else (62, 0)
                auxiliary = (f'bias_{name}_{group}.csl' if name in ('q', 'k', 'v') and ordinal == 0 else '<empty>')
                weights = f'weights_{x}_{y}.csl' if name != 'kv' else '<empty>'
    ...
    assert len(occupied) == 1176 and (62, 0) not in occupied
    lines = ['const memcpy=@import_module("<memcpy/get_params>",.{.width=64,.height=20});',
             'layout{@set_rectangle(64,20);']
    for y in range(20):
        for x in range(64):
            default = (4 if (x, y) == (62, 0) else 0, 0, 0, 2, 'WEST', 'EAST', 0, 62, 0,
                       '<empty>', 'controller_aux.csl' if (x, y) == (62, 0) else '<empty>')
    ...
    for color, left, right, top, bottom, period, extent in trees:
        lines.append(f'@set_color_config(62,0,@get_color({color}),.{{.routes=.{{.rx=.{{RAMP}},.tx=.{{WEST}}}}}});')
        for y in range(bottom+1):
            for x in range(left, 62 if y == 0 else right+1):
                ...
                lines.append(f'@set_color_config({x},{y},@get_color({color}),.{{.routes=.{{{routes}}}{filtering}}});')
...
def explicit_layer(weights, hidden):
...
def prepare(source_root=None, compile_parallelism=8):
    ...
    reference = ROOT / 'evidence/reference-f32'
    ...
    canonical = torch.from_numpy(source['model.embed_tokens'][0, :3].copy())
    changed = state['model.embed_tokens.weight'][changed_ids].float()
    ...
    for sequence, hidden in enumerate([canonical, changed]):
        with torch.inference_mode():
            nodes, keys, values, frequency = explicit_layer(state, hidden)
    ...
    files = [(Path(__file__), 'driver.py'), (source_root / 'tools/sdk_probe.py', 'executor.py'),
             ...
             (source_root / 'csl/programs/resident_decoder/kernel.csl', 'kernel.csl'),
             (source_root / 'csl/programs/resident_decoder/vector.csl', 'vector.csl'),
             (source_root / 'csl/programs/resident_decoder/rope.csl', 'rope.csl'),
             (source_root / 'csl/programs/resident_decoder/kv.csl', 'kv.csl'),
             (source_root / 'csl/programs/resident_decoder/line.csl', 'line.csl'),
             (source_root / 'csl/programs/resident_decoder/input.csl', 'input.csl'),
             (source_root / 'csl/programs/resident_decoder/packets.csl', 'packets.csl'),
    ...
    config = dict(cases=cases, changed_token_ids=changed_ids, compile_parallelism=compile_parallelism,
                  ...
                  scope='Complete native layer0 with1044 original weight tiles, biases/norms/RoPE,132 persistent KV PEs and device-only intermediate computation. Three canonical prefix positions, reset to two changed token IDs. Not24layers/full vocabulary/2048model trajectory.')
...
if __name__ == '__main__':
    ...
    if args.project_root:
        ...
        sys.path.insert(0, str(ROOT / 'src'))
    if args.prepare:
        prepare(args.source_root.resolve() if args.source_root else None, args.compile_parallelism)
    elif args.worker:
