...
def prepare(parent, source, project):
    ...
    for path, name in [(Path(__file__), 'driver.py'), (source / 'tools/topology_sdk_core.py', 'sdk_core.py'),
                       (source / 'tools/partition_executor.py', 'executor.py'),
    ...
    (out / 'config.json').write_text(json.dumps(dict(parent=str(parent),
        ...
        timing_scope='SDK core construction then selected memory extraction/comparison/NPZ save; initial artifact hashing excluded. Concurrent with vocabulary compilation, not isolated performance. Not a full151936logit-read benchmark.',
