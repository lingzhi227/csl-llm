...
def prepare(projection):
    ...
    for src,dst in [(Path(__file__),'driver.py'),(ROOT/'tools/sdk_probe.py','executor.py'),(ROOT/'tools/bf16_transport.py','bf16_transport.py'),(ROOT/'csl/kernels/local_gemv_bf16_f32_colmajor.csl','kernel.csl'),(ROOT/'csl/runtime/line_projection_f32.csl','line.csl'),(ROOT/'configs/precision.json','precision.json'),(ROOT/'src/csl_llm/projection_regions.py','regions.py')]:shutil.copy2(src,out/dst)
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--prepare',action='store_true');p.add_argument('--projection',choices=['down'],default='down');p.add_argument('--worker',type=Path);p.add_argument('--execute',type=Path);a=p.parse_args()
