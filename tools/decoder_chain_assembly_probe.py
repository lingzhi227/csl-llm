"""Freeze complete original two-layer audits for guarded intact-ELF assembly.
...
def prepare(audits, source, project):
    ...
    from decoder_chain_assemble import complete_coverage
    ...
    out = project / 'evidence' / ('decoder-chain-assembled-' + datetime.datetime.now(
        datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
    out.mkdir()
    for root, row in zip(audits, records):
        shutil.copytree(root, out / row['path'], ignore=shutil.ignore_patterns('__pycache__'))
    names = ['tools/decoder_chain_assemble.py', 'tools/model_partition_contract.py',
             'tools/sdk_probe.py', 'tools/elf_partition.py', 'tools/compare_elf_images.py',
             'tools/decoder_chain_audit_probe.py', 'tools/model_initializer_audit.py',
    ...
    (out / 'config.json').write_text(json.dumps(dict(audits=records,
        timeout_seconds=300, rss_limit_kib=4*1024*1024,
        scope='Original two-layer static intact-ELF assembly and loaded initial bytes only. No compiler, '
              'SdkRuntime or neural execution. Includes actual ELF symbol qualification.'), indent=2)+'\n')
...
def worker(root):
    ...
    from decoder_chain_assemble import assemble
    from decoder_chain_symbols import audit as audit_symbols
    verify(root)
    ...
    for row in config['audits']:
        ...
        audits.append(path)
    print('validate-compose-and-audit-original-two-layer', flush=True)
    result = assemble(audits, root / 'out')
    print('audit-actual-two-layer-symbols', flush=True)
    symbols = audit_symbols(root / 'out/bin')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source-root', type=Path, default=ROOT / 'support/two_layer')
