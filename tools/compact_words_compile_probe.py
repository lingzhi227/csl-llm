...
def prepare(parent, project, source):
    ...
    (out / 'weights.csl').write_text('const bytes=@get_array("'+encoded+'");\n'+'''fn unpack() [7168]u32 {
...
''')
    (out / 'pe.csl').write_text('param memcpy_params;const sys=@import_module("<memcpy/memcpy>",memcpy_params);const initial=@import_module("weights.csl");var weights=initial.values;const wp:[*]u32=&weights;fn noop()void{sys.unblock_cmd_stream();}comptime{@export_symbol(wp,"weights");@export_symbol(noop);}\n')
    (out / 'layout.csl').write_text('const memcpy=@import_module("<memcpy/get_params>",.{.width=1,.height=1});layout{@set_rectangle(1,1);@set_tile_code(0,0,"pe.csl",.{.memcpy_params=memcpy.get_params(0)});@export_name("weights",[*]u32,false);@export_name("noop",fn()void);}\n')
    ...
    (out / 'config.json').write_text(json.dumps(dict(parent=parent.name, parent_manifest_sha256=digest(parent / 'manifest.json'),
        source_packed_sha256=digest(path), tile_index=0, byte_offset=0, bytes=28672,
