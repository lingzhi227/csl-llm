"""Freeze and compile the complete resident model's uninitialized CSL graph.

This is a compiler/resource check, never an initialized inference acceptance.
All original matrix/KV owners are present; no SdkRuntime is constructed.
"""
import argparse
import datetime
import hashlib
...
def prepare(source, project, weight_storage='u32', static_inputs=None):
    ...
    (out / 'config.json').write_text(json.dumps(dict(
        width=193, height=210, matrix_tiles=34552, kv_tiles=3168,
...
if __name__ == '__main__':
    ...
    if args.prepare:
        prepare(args.source_root.resolve(), args.project_root.resolve(), args.weight_storage,
                args.static_inputs.resolve() if args.static_inputs else None)
    elif args.worker:
        worker(args.worker.resolve())
    else:
        sys.path.insert(0, str(args.execute.resolve()))
        from executor import execute
        execute(args.execute.resolve(), 900, rss_limit_kib=8*1024*1024)
