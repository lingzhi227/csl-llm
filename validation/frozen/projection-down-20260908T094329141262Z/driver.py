...
def prepare(projection):
    ...
    model=ROOT/'models/qwen2.5-0.5b-7ae5576/model.safetensors';trace=ROOT/'evidence/reference-f32-greedy-20260908T0554/arithmetic/step-000.npz';state=load_file(model);ref=np.load(trace)
    ...
    for src,dst in [(Path(__file__),'driver.py'),(ROOT/'tools/sdk_probe.py','executor.py'),(ROOT/'tools/bf16_transport.py','bf16_transport.py'),(ROOT/'csl/kernels/local_gemv_bf16_f32_colmajor.csl','kernel.csl'),(ROOT/'csl/runtime/line_allreduce_f32.csl','line.csl'),(ROOT/'configs/precision.json','precision.json'),(ROOT/'src/csl_llm/regions.py','regions.py')]:shutil.copy2(src,out/dst)
...
var weights=@zeros([7168]u32);var input=@zeros([112]f32);var partial=@zeros([128]f32);var result=@zeros([128]f32);var progress=@zeros([1]u32);var timing=@zeros([6]u16);var started=@zeros([3]u16);var ended=@zeros([3]u16);
fn reset() void {progress[0]=0;sys.unblock_cmd_stream();}
fn compute() void {clock.enable_tsc();clock.get_timestamp(&started);kernel.compute(@ptrcast([*]u16,&weights),&input,&partial);line.start(&partial,&result);}
...
const wp:[*]u32=&weights;const xp:[*]f32=&input;const rp:[*]f32=&result;const pp:[*]u32=&progress;const tp:[*]u16=&timing;
...
def worker(root):
    ...
    weight_pairs = pack_bf16_pairs(data['packed_weights'])
    try:
        stage('load-resident-weights');runner.memcpy_h2d(ids['weights'],weight_pairs,0,0,w,h,7168,streaming=False,data_type=MemcpyDataType.MEMCPY_32BIT,order=MemcpyOrder.ROW_MAJOR,nonblock=False)
        for epoch,name in enumerate(c['cases']):
            ...
            for x in sorted({point[0] for point in c['roots']}):
                columns[x]=(read('result',x,0,1,h,128).reshape(h,128),read('timing',x,0,1,h,6).reshape(h,6))
                column_file=root/f'column-read-{epoch}-{x}.npz'
                np.savez(column_file,output=columns[x][0],timing=columns[x][1]);column_hashes[column_file.name]=digest(column_file)
            for x,y in c['roots']:
                outputs.append(columns[x][0][y]);timing.append(columns[x][1][y])
            ...
            t=timing.astype(np.uint64);assert np.all(t<65536);ticks=lambda a:a[:,0]+(a[:,1]<<16)+(a[:,2]<<32);cycles=(ticks(t[:,3:])-ticks(t[:,:3]))&np.uint64((1<<48)-1);assert np.all((cycles>0)&(cycles<1<<40));reports.append(dict(name=name,relative_l2=l2,relative_peak=peak,official=official_check,root_cycles=cycles.tolist(),column_readback_sha256=column_hashes,actual_sha256=digest(root/f'actual-{epoch}.npz')));(root/'results.json').write_text(json.dumps(dict(success=False,cases=reports),indent=2)+'\n')
    finally:runner.stop()
    (root/'results.json').write_text(json.dumps(dict(success=True,cases=reports,scope=c['scope']),indent=2)+'\n')


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--prepare',action='store_true');p.add_argument('--projection',choices=['up','gate','down'],default='up');p.add_argument('--worker',type=Path);p.add_argument('--execute',type=Path);a=p.parse_args()
