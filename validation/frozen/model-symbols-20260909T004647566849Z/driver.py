...
def prepare(audited, source, project, *, diagnostics=False):
    ...
    names = ['tools/model_symbols.py', 'tools/sdk_probe.py',
             'src/csl_llm/__init__.py', 'src/csl_llm/decoder_layout.py', 'src/csl_llm/regions.py']
    if diagnostics:
        names.append('tools/decoder_chain_symbols.py')
    for name in names:
        target = out / name
...
def worker(root):
    ...
    symbols = audit(root / 'audited/compiled/out/bin', diagnostics=config.get('diagnostics', False))
...
if __name__ == '__main__':
    ...
    if args.audit:
        prepare(args.audit.resolve(), args.source_root.resolve(), args.project_root.resolve(), diagnostics=args.diagnostics)
    elif args.worker:
