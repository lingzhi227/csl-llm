...
def layout(width, height, regions, exports, *, group_identity=True):
    """Emit regions; coalesce group parameters only for group-invariant PE code."""
    occupied = {}
    for group, region in enumerate(regions):
        ...
        for ordinal in range(region.participants):
            ...
            occupied[point] = (group if group_identity else 0, ordinal, *region.neighbors(ordinal), region.participants)
