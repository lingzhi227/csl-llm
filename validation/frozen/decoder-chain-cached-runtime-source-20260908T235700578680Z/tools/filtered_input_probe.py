...
def prepare():
    ...
    trace=ROOT/'evidence/reference-f32-greedy-20260908T0554/arithmetic/step-000.npz';r=np.load(trace);x=(torch.nn.functional.silu(torch.from_numpy(r['model.layers.0.mlp.gate_proj'][0,-1]))*torch.from_numpy(r['model.layers.0.mlp.up_proj'][0,-1])).numpy()
    ...
    for src,dst in [(Path(__file__),'driver.py'),(ROOT/'tools/sdk_probe.py','executor.py'),(ROOT/'csl/runtime/filtered_input.csl','input.csl')]:shutil.copy2(src,out/dst)
