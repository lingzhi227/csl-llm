...
def make_layout(regions, *, origin=(0, 0), layer=0, fragment=False, next_destination=None, initialization='static', code_file='pe.csl', coordinate_identity=False, weight_storage='u32', static_data=None):
    from csl_llm.static_data import bindings
    data_bindings = bindings(static_data)
    remaining_bindings = set(data_bindings)
    if data_bindings and (initialization != 'sdk' or weight_storage != 'u8'):
        raise ValueError('Explicit native data bindings require the SDK/u8 layout mode')
    if weight_storage not in ('u32', 'u8'):
        raise ValueError('Weight storage must be original packed u32 or native u8 bytes')
    ...
    """
    if coordinate_identity and (initialization != 'sdk' or origin[0] % 64 or origin[1] % 20
                                or not (0 <= origin[0] < 192 and 0 <= origin[1] < 160)):
        raise ValueError('Coordinate identity requires a fixed aligned model region and runtime identity storage')
    ...
    for y in range(by, by+20):
        for x in range(bx, bx+64):
            ...
            bound = (x, y) in data_bindings
            if bound:
                data = data_bindings[x, y]
                matrix_role = role in (1, 2, 3, 5, 6, 7, 8)
                needs_auxiliary = role == 4 or (role in (5, 6, 7) and ordinal == 0)
                if (not (matrix_role or role == 4) or
                        (data.weight_file is not None) != matrix_role or
                        (data.auxiliary_file is not None) != needs_auxiliary):
                    raise ValueError(f'Static data does not match decoder role at {(x,y)}')
                weight = data.weight_file or '<empty>'
                aux = data.auxiliary_file or '<empty>'
                remaining_bindings.remove((x, y))
            ...
            if initialization == 'sdk':
                params += f',.static_weights={str(bound).lower()},.runtime_identity=true'
            ...
            lines.append(f'@set_tile_code({x},{y},"{code_file}",.{{{params}}});')
    ...
    if remaining_bindings:
        raise ValueError('Static data lies outside this decoder region')
    if fragment:
        return '\n'.join(lines) + '\n'
    for name, kind in [('weights', weight_storage), ('input', 'f32'), ('result', 'f32'), ('context', 'f32'),
