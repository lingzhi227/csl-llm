...
def prepare(source, project, full=False, channels=1):
    width, height, tail = (193, 50, 11) if full else (17, 2, 2)
    if not 1 <= channels <= min(height, 16):
        raise ValueError('SDK channels must fit the row count and16channel maximum')
...
def worker(root):
    ...
    command = ['cslc', 'layout.csl', '--arch=wse3', f'--fabric-dims={width+7},{height+2}',
               ...
               '--memcpy', f"--channels={config.get('memcpy_channels', 1)}", '--max-parallelism=1', '--dump-dsr-alloc-graph']
...
if __name__ == '__main__':
    ...
    if args.prepare:
        prepare(args.source_root.resolve(), args.project_root.resolve(), args.full_fabric, args.channels)
    elif args.worker:
