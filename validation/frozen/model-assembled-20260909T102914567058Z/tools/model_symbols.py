...
def resolve(objects, logical, size, *, alignment=None):
    if alignment is None:
        alignment = 2 if logical == 'timing' else 4
    if type(alignment) is not int or alignment not in (1, 2, 4):
        raise ValueError('Explicit object alignment must be 1, 2 or 4 bytes')
...
def audit(directory, *, diagnostics=False):
    ...
    if type(diagnostics) is not bool:
        raise ValueError('Diagnostic ABI selection must be boolean')
    if diagnostics:
        import decoder_chain_symbols
    ...
    for path in sorted(Path(directory).glob('*.elf')):
        ...
        for px, py in points:
            ...
            if y < 160:
                ...
                if diagnostics:
                    fields = decoder_chain_symbols.requirements((x, y), tile,
                        (x, y) in controllers, endpoint=False)
                    required.update({name: size for name, (size, _) in fields.items()})
                    alignments.update({name: alignment for name, (_, alignment) in fields.items()})
            else:
                ...
                if diagnostics:
                    # Every vocabulary shard holds the complete allreduce;
                    # its normal readback is independently checked bitwise.
                    if matrix:
                        required['result'] = 512
                    if matrix or (x, y) == (192, 160):
                        required.update({'packets.sending': 2, 'packets.receiving': 2})
                        alignments.update({'packets.sending': 2, 'packets.receiving': 2})
                    if (x, y) == (192, 160):
                        required['boundary.invocation'] = 4
                        for name in ('received_count', 'sent_count', 'pending_count', 'command', 'started', 'sending'):
                            required['boundary.'+name] = 2
                            alignments['boundary.'+name] = 2
            for logical, size in required.items():
                row = resolve(objects, logical, size, alignment=alignments.get(logical))
                actual = row['name']
    ...
    dependencies = (decoder_layout, regions, decoder_chain_symbols) if diagnostics else (decoder_layout, regions)
    return dict(passed=True, application_coordinates=len(seen), images=records, diagnostics=diagnostics,
