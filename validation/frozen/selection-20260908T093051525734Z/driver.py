...
def prepare():
    ...
    model=ROOT/'models/qwen2.5-0.5b-7ae5576/model.safetensors';trace=ROOT/'evidence/reference-f32-greedy-20260908T0554/arithmetic/step-000.npz';base=151552
    ...
    for src,dst in [(Path(__file__),'driver.py'),(ROOT/'tools/sdk_probe.py','executor.py'),(ROOT/'tools/bf16_transport.py','bf16_transport.py'),(ROOT/'csl/kernels/tied_embedding_f32.csl','embedding.csl'),(ROOT/'csl/kernels/argmax_f32.csl','argmax.csl')]:shutil.copy2(src,out/dst)
    (out/'config.json').write_text(json.dumps(dict(base=base,cases=cases,weight_transfer='BF16 pairs via raw u32 H2D/D2H, no numeric conversion',model_sha256=digest(model),trace_sha256=digest(trace),scope='One actual128x112 tied embedding tile including EOS row; exact lookup and128-value FP32 argmax with global-index pair merge. Not full vocabulary reduction or model generation.'),indent=2)+'\n')
...
''')
    names=[('weights','u32'),('values','f32'),('candidate','f32'),('lookup','f32'),('best','f32'),('index','u32'),('control','u32'),('progress','u32')]
...
def worker(root):
    ...
    weight_pairs = pack_bf16_pairs(data['packed'])
    try:
        runner.memcpy_h2d(ids['weights'],weight_pairs,0,0,1,1,7168,streaming=False,data_type=MemcpyDataType.MEMCPY_32BIT,order=MemcpyOrder.ROW_MAJOR,nonblock=False)
        weight_words=np.zeros(7168,np.uint32);runner.memcpy_d2h(weight_words,ids['weights'],0,0,1,1,7168,streaming=False,data_type=MemcpyDataType.MEMCPY_32BIT,order=MemcpyOrder.ROW_MAJOR,nonblock=False);np.save(root/'actual-weights.npy',weight_words);assert np.array_equal(weight_words,weight_pairs)
    ...
    (root/'results.json').write_text(json.dumps(dict(success=True,cases=reports,weight_readback_sha256=digest(root/'actual-weights.npy'),scope=c['scope']),indent=2)+'\n')
