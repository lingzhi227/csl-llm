...
def prepare(assembled, reference, build, source, project, timeout_seconds):
    ...
    names = ['tools/sdk_probe.py', 'tools/sdk_core.py', 'tools/sdk_lifecycle.py', 'tools/model_reference.py',
        ...
        def receipt(errors):
            (directory/'runtime-evidence.json').write_text(json.dumps(dict(
                files={name: sha(directory/name) for name in ('out.core', 'normal-d2h.npz', 'normal-report.json')
                       if (directory/name).is_file()}, compiled_files_sha256=sha(root/'compiled-files.json'),
                       **errors), indent=2)+'\n')
        stop_and_record(runner, stage=stage, record=receipt,
                        primary_error=primary_error, references=[reference])
