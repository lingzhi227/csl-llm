...
def contract_symbols(contract, composition):
    ...
    for row in contract['images']:
        ...
        # Logical names are an explicit, hash-bound mapping from the actual ELF
        # symbol audit. Never infer private names or addresses while reading core.
        aliases = row.get('logical_symbols', {})
        for logical, actual in aliases.items():
            if (not isinstance(logical, str) or not logical or
                    not isinstance(actual, str) or actual not in row['required_symbols']):
                raise ValueError('Logical alias must reference an audited actual symbol')
            target = entries[actual]
            if logical in entries and entries[logical] != target:
                raise ValueError('Logical alias conflicts with an actual symbol')
