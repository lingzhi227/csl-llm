...
This helper has host scheduling/failure tests only. It does not by itself provide a
...
qualify the actual stopped core and immutable weights.

`model_generation_probe.py` now implements that guarded owner in authoring
source, with independent source and host-test review. Preparation requires an
explicit budget, the complete assembly and actual successful first single-token
integration. `model_integration_admission.py` pins all 17 dependencies copied
by the reviewed first-run producer, verifies its entire frozen input manifest,
normal/core output hashes and clean lifecycle receipts, and requires the same
assembly identity. Preparation copies first-run metadata with the verified
actual-output digests into the new candidate; it does not substitute metadata
for the actual first-run verification. Current PID absence and independent
acceptance remain separate dispatch checks.

The generation worker checks all four requests, reset-repeat equality, final
actual core, complete original initializers and exact runtime artifact file set
before writing success. A first-run receipt from the old lifecycle source is
rejected. No generation candidate has been prepared or executed, and no
wall-time budget has been selected for it.
